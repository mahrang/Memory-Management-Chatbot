...
/* edge is a unique pointer, as shown in line 177 above.  You can retrieve the internal raw pointer of edge with get().  edge.get() gives you the address of edge. You can see an example of get() in file "Part 6 - Smart Pointers" under the heading "The Unique pointer". */
                            ...
                            /* Next line shows GraphNodes exclusively owns the outgoing GraphEdges for task 4.  It also shows move semantics are used when transferring ownership from class ChatLogic, where all instances of GraphEdge are created, into instances of GraphNode for task 4. */
